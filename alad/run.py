...
def train_and_test(dataset, nb_epochs, degree, random_seed, label,
    ...
    with sv.managed_session(config=config) as sess:
        ...
        for t in range(nr_batches_test):
            ...
            #print("scores_l1: ", scores_l1)
            print("scores_l1 average: ", np.average(scores_l1))
            print("scores_l1 max: ", np.amax(scores_l1))
            scores_l2 += sess.run(score_l2, feed_dict=feed_dict).tolist()
            #print("scores_l2: ", scores_l2)
            print("scores_l2 average: ", np.average(scores_l2))
            print("scores_l2 max: ", np.amax(scores_l2))
            scores_fm += sess.run(score_fm, feed_dict=feed_dict).tolist()
            #print("scores_fm: ", scores_fm)
            print("scores_fm average: ", np.average(scores_fm))
            print("scores_fm max: ", np.amax(scores_fm))
