...
def do_hist(scores, true_labels, directory, dataset, random_seed, display=False):
    ...
    print("length idx_inliers :", len(scores[idx_inliers]))
    print("length idx_outliers :", len(scores[idx_outliers]))
    plt.hist(scores[idx_outliers])
    print("passed! nothing but the inlier scores ")
    plt.hist(scores[idx_inliers])
    print("passed! nothing but the inlier scores ")
    plt.hist(scores[idx_inliers], label="Normal samples", density=True)
    print("passed! no bins")
